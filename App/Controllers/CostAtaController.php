...
class CostAtaController extends Controller
    {
        ...
        public function resume($params)
        {
            $costAtaId = $params[0];
            $costAtaDAO = new CostAtaDAO();
            $this->setViewParam('headerCostAta', $costAtaDAO->findId($costAtaId));

            $costAtaDetailDAO = new CostAtaDetailDAO();
            $this->setViewParam('itens', $costAtaDetailDAO->findAll($costAtaId));
        ...
        public function delete($params)
        {
           if($params[0])
           {
               ...
               $costAta->setId($id);     
                   ...
                   $costAtaDAO = new CostAtaDAO();
                   if($costAtaDAO->deleteCostAta($costAta))
                   {
                        Session::unsetMessage();
                        Session::setMessage('ata excluido com sucesso!');
                        $this->index();
                   }
                   else 
                   {
                        Session::unsetErro();
                        Session::setErro('Nao foi possivel Excluir verifique os dados e tente novamente');
                        $this->redirect('/costAta/index'); 
                   }               
           }
           else 
        ...
        public function updateDetail()
        {
            ...
            if ($_POST) {
                ...
                $costAtaId              = filter_var($_POST['cost_ata_id'], FILTER_SANITIZE_SPECIAL_CHARS);
                $costAtaPr              = filter_var($_POST['cost_ata_pr'], FILTER_SANITIZE_SPECIAL_CHARS);
                $costAtaIdClient        = filter_var($_POST['cost_ata_id_client'], FILTER_SANITIZE_SPECIAL_CHARS);
                $id                     = $_POST['id'];
                $numberItem             = $_POST['txt_number_item'];
                ...
                if ($costAtaDetailDAO->updateCostDetail($costAtaDetail)) {
                    Session::unsetMessage();
                    Session::setMessage('itens atualizado com sucesso');
                ...
                Session::unsetErro();
                Session::setErro('Nao foi possivel atualizar verifique os dados e tente novamente');
                $this->redirect('/costAta/index');
            }
        }
